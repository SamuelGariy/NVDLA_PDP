...
def generate_input_and_solutions(Op,n_arrays,dtype,kernel_height,kernel_width,input_file, solution_file):
    ...
    with open(input_file, "w") as file1:
            ...
            file1.write(tabs + f"\"pdp_csb2pdp_data\": {str(kernel_size_input)}, \n")
            ...
            for k in range(len(results)):
                file1.write("\t\t,{\n")
                file1.write(tabs + f"\"instr No.\": {k + 2}, \n")
                file1.write(tabs + f"\"pdp_pdp_last_input_batch\": {str(k == len(results) - 1).lower()}, \n")
                file1.write(tabs + f"\"pdp_pdp_input_vld\": true, \n")

                for i in range(len(arr) - 1):
                    file1.write(tabs + f'\"pdp_pdp_input_{i}\": {results[k][i]}, \n')
                file1.write(tabs + f'\"pdp_pdp_input_{len(arr) - 1}\": {results[k][(len(arr) - 1)]} \n')    
                file1.write("\t\t}\n")
...
def main():
    if len(sys.argv) != 8:
        ...
        print("Usage: input_generator.py [ max | min | mean ]  n_arrays [ int8 | int16 ] kernel_height kernel_width input_file solution_file")
        exit
    
    _,Op, n_arrays, dtype, kernel_height, kernel_width,input_file,solution_file = sys.argv
  
    generate_input_and_solutions(Op,n_arrays,dtype,kernel_height,kernel_width,input_file, solution_file)
    print("done")
