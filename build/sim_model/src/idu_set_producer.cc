#include <pdp.h>
bool pdp::decode_pdp_set_producer() {
...
void pdp::update_pdp_set_producer() {
auto local_var_1 = pdp_csb2pdp_data.range(0, 0);
auto local_var_1_nxt_holder = local_var_1;
pdp_pdp_s_producer = local_var_1_nxt_holder;
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "set_producer state updates:" << std::endl;
instr_update_log << "    pdp_pdp_s_producer => " << std::hex << "0x" << pdp_pdp_s_producer << std::endl; 
