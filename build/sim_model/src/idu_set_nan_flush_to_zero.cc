#include <pdp.h>
bool pdp::decode_pdp_set_nan_flush_to_zero() {
auto local_var_1 = pdp_csb2pdp_addr.range(11, 0);
sc_biguint<12> local_var_2 = 40;
...
auto& univ_var_10 = local_var_15;
return univ_var_10;
}
void pdp::update_pdp_set_nan_flush_to_zero() {
auto local_var_1 = pdp_csb2pdp_data.range(0, 0);
auto local_var_1_nxt_holder = local_var_1;
pdp_group0_pdp_d_nan_to_zero = local_var_1_nxt_holder;
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "set_nan_flush_to_zero state updates:" << std::endl;
instr_update_log << "    pdp_group0_pdp_d_nan_to_zero => " << std::hex << "0x" << pdp_group0_pdp_d_nan_to_zero << std::endl; 
